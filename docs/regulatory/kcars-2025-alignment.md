...
**Update (29 May 2026) — KCAA published-regs page review.** The KCAA "Regulations 2025" page (`kcaa.or.ke/published-regs-2025`) states **29 revised regulations are gazetted** with a further set awaiting publication. Cross-checked against the platform's binding-law instruments (LN 29/30/31/37/40/41/42 + LN 32/47/50): **all remain consistent** with what is already verified against Kenya Law — no code change required. **One open flag (domain-critical):** the page's index is ambiguous on whether the **Civil Aviation (Fatigue Management) Regulations** is now gazetted (it appears in both "published" and "awaiting publication" groupings; the RIA still sits on transport.go.ke as a 2024 instrument). Per CLAUDE.md, FTL is safety-critical, so the FCTS **retains its no-FTL posture** until the specific Fatigue Management Legal Notice is supplied and read. FTL as OM content is already captured at LN 42 Third Schedule §2.1.2 (numeric limits derive from the operator's approved scheme). **Action: Capt. Ng'ong'a to confirm Fatigue Management status / share the LN if gazetted**, before any FTL modelling.
